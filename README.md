...
For trusted HTTPS certificates in your browser, install mkcert on your host system:
...
1. **Install mkcert on Windows:**
   ...
   **Download from official GitHub releases:**
   - Go to [mkcert releases](https://github.com/FiloSottile/mkcert/releases)
   - Download `mkcert-v*-windows-amd64.exe`
   - Rename to `mkcert.exe` and move to a folder in your PATH
   
   **Or using PowerShell (direct download):**
   ```powershell
   ...
   Move-Item "mkcert.exe" "C:\Windows\System32\mkcert.exe"
   ```
...
3. **Verify installation:**
   ```powershell
   mkcert -CAROOT
   # Should show: C:\Users\[USERNAME]\AppData\Local\mkcert
   ```

**That's it!** The Docker container will automatically detect and use your Windows mkcert CA.
...
4. **Mount your home directory when running Docker:**
   ```bash
   # Add this volume mount to docker-compose.yml hub service:
   volumes:
     - $HOME:/host-home:ro
   ```
...
2. **Install mkcert:**
   ...
   **Option B: Direct download from GitHub (recommended):**
   ```bash
   # Download latest release
...
3. **Mount your home directory when running Docker:**
   ```bash
   # Add this volume mount to docker-compose.yml hub service:
   volumes:
     - $HOME:/host-home:ro
...
### Alternative: Container-Only Certificates

If you prefer not to install mkcert on your host:

1. **Skip the mkcert installation** - the container will create its own CA
2. **Accept browser warnings** - click "Advanced" → "Accept Risk and Continue"
3. **Container-only CA** - certificates will only be trusted inside the container

**Note:** With container-only certificates, you'll see SSL warnings in your browser, but the site will still work for development.
